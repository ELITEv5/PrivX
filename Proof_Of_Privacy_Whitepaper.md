...
1. **Generate Secret:**  
   A 32-byte random value (`secret`).
2. **Derive Hash:**  
   `hash = keccak256(abi.encode(secret))`
3. **Deposit PRIVX:**  
   Lock amount under `hash`.
4. **Commit (Receiver):**  
   Receiver commits using `keccak256(abi.encode(secret, receiver))`
5. **Withdraw:**  
   Reveal `secret` + fresh wallets for disbursement.  
   Contract verifies hash match → sends funds.
6. **Reclaim (Optional):**  
   After 7 days of inactivity, anyone may reclaim expired deposits.
...
Every cycle decreases total circulating supply while rewarding participation.
This creates a self-balancing deflationary feedback loop — where using the privacy layer strengthens the economy itself.
...
20% of fee routed to Vault

Reclaimer earns 5% bounty as incentive
...
Framework: Ethers.js / Minimal UMD frontend

Network: PulseChain RPC multi-provider failover

Web Interface: Static, PWA-enabled, open-source (V13)

Contracts: Immutable, verified on-chain

Audit Status: Community reviewed, deterministic build
